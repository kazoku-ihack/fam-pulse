...
| `RIDER_FALLBACK_ADDR` | Workaround for the real Rider deployment rejecting Protosure's actual `attesterSig` (a confirmed digest mismatch — see `knowledge.md`). When set, `protosure-direct` transfers target this address instead of `RIDER_ADDR`, using the plain single-signature `submitTrigger` (a `MimamorParametricBind` instance, deployed via `chain/deploy-rider-fallback-bind.js` wrapping the real `JPYC_ADDR` token, verifying against a settable bind address set to `PAYOUT_ADDR` rather than its own address — required, since Protosure always signs against `PAYOUT_ADDR` regardless of where this fallback lives) — no `oracleSig` needed for this path. Unset (default) → behavior unchanged, still targets the real Rider contract. |
