...
| `ATTESTER_ADDRESS` (falls back to `REGISTERED_SIGNER`) | `POST /v1/jpyc/preTransferHash` independently recovers the attester's address from `attester.payload_hash`/`signature` and requires it to match — protects against persisting (and later, on a Rider submit, wasting gas on) an attestation from an unexpected signer. `ATTESTER_ADDRESS` is checked first; `REGISTERED_SIGNER` is only a fallback. Unset (both) → `503 ATTESTER_ADDRESS_NOT_CONFIGURED`; mismatch → `422 ATTESTER_ADDRESS_MISMATCH`. |
