...
- **Scope change (2026-07-30): `POST /v1/jpyc/transfer` now calls a different, externally-deployed
  ...
  - **Workaround shipped (2026-07-30): `RIDER_FALLBACK_ADDR`** — a freshly-deployed, *unmodified*
    ...
    already held ~937,000 tokens). Routing (`isChainReadyFor`/`executePayoutOnChain` in
    `src/routes/payments.js`): if `RIDER_FALLBACK_ADDR` is set, `protosure-direct` attestations go
    through `chain.getRiderFallbackContract()` with the plain single-sig `submitTrigger` (same
    shape as the non-`protosure-direct` path — no `oracleSig`, no `ORACLE_SIGNER_PRIVATE_KEY`
    needed for this branch) instead of the dual-sig Rider path; unset (default), behavior is
    unchanged.
    - **CORRECTED same day, superseded by the bind-address fix below: this first version only
      worked for attestations generated by the rehearsal script/endpoint, not real Mendix ones.**
      The assumption above — that `REGISTERED_SIGNER` being registered on the fallback contract
      was sufficient — missed that vanilla `MimamorParametric.sol`'s `computeInner` binds
      `address(this)` (i.e. the fallback contract's *own* address) into the digest. Confirmed live
      against a real user-reported attestation (`573f7bcb-...`, `triggerRef:"TRG-004-20260730-24"`):
      recomputing its digest bound to `PAYOUT_ADDR` matched `payload_hash` exactly and recovered to
      the real `REGISTERED_SIGNER`; bound to the fallback contract's own address, it didn't match at
      all. Submitting that real attestation to the first fallback contract reverted `SIGNER_MISMATCH`
      ("signer not registered on contract") — not because the signer wasn't registered, but because
      the wrong digest was being verified. The rehearsal script/endpoint never caught this because
      *they* controlled the digest generation and deliberately bound it to the fallback address to
      match — real Mendix traffic has no such flexibility, since Protosure always signs against
      `PAYOUT_ADDR` regardless of where any fallback lives.
  - **Fix (2026-07-30, same day): `MimamorParametricBind.sol` + redeployed `RIDER_FALLBACK_ADDR`.**
    New contract (`src/chain/contracts/MimamorParametricBind.sol`, deployed via
    `src/chain/deploy-rider-fallback-bind.js`), identical to `MimamorParametric.sol` except
    `computeInner` binds a settable `bindAddress` (an added state variable + `setBindAddress`
    owner-only setter) instead of `address(this)`. Deployed with `bindAddress = PAYOUT_ADDR` — so
    it verifies signatures against the digest Protosure *actually* signs, while living at its own,
    different address and still holding/paying out the real `JPYC_ADDR` token. Confirmed live:
    resubmitted the same real attestation (`573f7bcb-...`) directly against the new contract —
    succeeded (`0x4d578f91...`, Snowtrace-visible). The rehearsal script/endpoint were updated to
    match: they now always sign against `PAYOUT_ADDR` (not the fallback contract's own address),
    which is simpler and correct for both self-generated and real attestations alike, since both
    now go through the identical bind target.
    - **Side effect of the verification above**: attestation `573f7bcb-...`'s on-chain nonce
      (`triggerRef:"TRG-004-20260730-24"`) was consumed directly via a scratch script during
      verification, bypassing the app — the app's own DB still shows it `status:"signed"`, but a
      real `/v1/jpyc/transfer` on that specific id will now fail `NONCE_ALREADY_USED` (permanent
      failure, releases cap ledger) rather than succeed, since it was already paid out on-chain
      outside the app's bookkeeping. One-off; doesn't affect any other attestation.
    - **Caveat, unchanged from the first version**: `amountJpy` still moves unscaled (see the
      `amountWei` bullet below) against an **18-decimal** token — e.g. `payout_amount=30000` moves
      `30000` raw units = `0.00000000000003` tokens, a real but dust-sized transfer. Fine for
      proving the pipeline produces a genuine, mined, Snowtrace-visible transaction; not fine if a
      JPY-equivalent amount ever needs to actually move — would need scaling added at the
      `jpyc.transfer()` call site only, never touching the digest-input `amountJpy` value. Still a
      stopgap, not a fix for the real Rider contract itself — revert to the Rider path by unsetting
      `RIDER_FALLBACK_ADDR` once Protosure/Rider's actual digest mismatch is reconciled upstream
      (that mismatch is unrelated to and unaffected by this fix — Rider is still a different,
  ...
  - **Repeatable rehearsal script: `src/chain/rehearse-protosure-direct.js`** — drives the real
    ...
    since this repo doesn't hold Protosure's real key. Confirmed by hitting live: the claimed
    `attester.signer` field must equal `ATTESTER_ADDRESS` — checked directly, not by recovery, so
    this is set explicitly rather than derived from the signing key. **The digest itself binds
    `PAYOUT_ADDR`** (`computeInner`'s `contractAddress` param) — same value as the request body's
    `contract_address` field, since `RIDER_FALLBACK_ADDR` now verifies against a settable
    `bindAddress` (= `PAYOUT_ADDR`) rather than its own address (`MimamorParametricBind.sol` — see
    the bind-address fix bullet above). An earlier version of this script signed against the
    fallback contract's own address instead, matching that first (non-bind) fallback deployment —
    updated when the fallback contract was replaced, since the two must always agree on what
    address the digest binds. Each run generates a fresh `trigger_ref` so repeats never collide on
    `NONCE_ALREADY_USED`.
  - **`POST /v1/jpyc/rehearseTransfer`** (`src/routes/payments.js`) — the same rehearsal flow as an
    ...
    `attestationId` (optional) — every signing detail (`STUB_SIGNER_PRIVATE_KEY`, `PAYOUT_ADDR` as
    the digest bind target, `ATTESTER_ADDRESS`/`REGISTERED_SIGNER`, `CHAIN_ID`) is read from
    server-side env at request time, and the generated attestation is
